 ...
 *  PR 3). One-shot at app mount; the launcher is env-controlled so it
...
/** Open a file from a pane's cwd in the configured IDE. Resolves a discrete
 *  status so the caller can surface the right toast — `disabled` (server
 *  has no IDE configured), `not-found` (path doesn't resolve to a file),
...
export async function openInIde(
  ...
  const r = await fetch(
    `${BASE}/open?session=${encodeURIComponent(session)}&index=${index}&path=${encodeURIComponent(path)}`,
    { method: "POST", headers: { ...csrfHeaders() } },
  );
  if (r.ok) return "ok";
