...
# ─── THI-139: cost aggregation ────────────────────────────────────────
# Pricing table in `services/pricing.py` is the source of truth; tests
# below pin the dollar amounts derived from it. If the table changes,
# update the expected values in lockstep.

def test_cost_zero_when_no_model_field(tmp_path: Path) -> None:
    # Existing fixtures (session-fresh / mixed / stale) carry no `model`
    # key — they predate the cost path. Token counts must still aggregate
    # cleanly (regression guard against breaking pre-THI-139 fixtures)
    # and cost stays at 0 with no unknown-models entries (empty model
    # string is dropped from the set, per the `if model` filter).
    projects = _seed_projects_dir(tmp_path, "session-fresh.jsonl")
    result = claude_usage.compute_claude_usage(projects_dir=projects, now=NOW)
...
def test_cost_for_single_sonnet_session(tmp_path: Path) -> None:
    # One record at Sonnet pricing:
    # 1000 input @ $3      = $0.003
    # 2000 cache_w @ $3.75 = $0.0075
    # 4000 cache_r @ $0.30 = $0.0012
    # 500 output @ $15     = $0.0075
    # total                  $0.0192
    projects = _seed_projects_dir(tmp_path, "session-priced-sonnet.jsonl")
    result = claude_usage.compute_claude_usage(projects_dir=projects, now=NOW)
    assert result.cost_usd == 0.0192
    assert result.unknown_models == []


def test_cost_sums_across_model_families(tmp_path: Path) -> None:
    # Three records (Opus, Sonnet, Haiku), each 1000 input + 500 output,
    # no cache. Per record:
    #   Opus:   1000 * $15/M + 500 * $75/M = $0.015 + $0.0375 = $0.0525
    #   Sonnet: 1000 * $3/M  + 500 * $15/M = $0.003 + $0.0075 = $0.0105
    #   Haiku:  1000 * $1/M  + 500 * $5/M  = $0.001 + $0.0025 = $0.0035
    #   total                                                   $0.0665
    projects = _seed_projects_dir(tmp_path, "session-priced-mixed.jsonl")
    result = claude_usage.compute_claude_usage(projects_dir=projects, now=NOW)
    assert result.cost_usd == 0.0665
    assert result.unknown_models == []


def test_cost_skips_unknown_model_and_surfaces_id(tmp_path: Path) -> None:
    # Two records: one Sonnet (cost), one unknown future model (no cost
    # but tokens still aggregate). The unknown id appears verbatim in
    # `unknown_models` so the UI can hint that pricing is incomplete.
    projects = _seed_projects_dir(tmp_path, "session-priced-unknown.jsonl")
    result = claude_usage.compute_claude_usage(projects_dir=projects, now=NOW)
    # Only the Sonnet record contributes cost: 1000 input + 500 output.
    # 1000*3 + 500*15 = 10500 micro$ → $0.0105.
    assert result.cost_usd == 0.0105
    # Both records still aggregated into the token totals (the pill stays
    # honest even when pricing is missing).
    assert result.input_tokens == 2000
    assert result.output_tokens == 1000
    assert result.unknown_models == ["claude-future-9-9-20991231"]
